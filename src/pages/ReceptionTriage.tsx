...
export default function ReceptionTriage() {
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      ...
      console.error("Erro ao carregar triagem da recepção:", error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Triagem (Lista)</h1>
          <p className="text-muted-foreground">Acompanhamento operacional da triagem</p>
        </div>
      ...
      {loading ? (
        <div className="text-muted-foreground">Carregando lista de triagem...</div>
      ) : (
        <ReceptionQueueList
